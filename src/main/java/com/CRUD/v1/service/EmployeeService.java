...
import com.CRUD.v1.builder.JsonResponseBuilder;
...
public class EmployeeService {
    ...
    public ResponseEntity<Object> createEmployee(Employee employee) {
        if (employee.getName() == null) {
            return builder.responseBuilder("Employee name cannot be null", HttpStatus.BAD_REQUEST, employee);
        } else if (employee.getEmail() == null) {
            return builder.responseBuilder("Employee email cannot be null", HttpStatus.BAD_REQUEST, employee);
        } else if (employee.getDepartment() == null) {
            return builder.responseBuilder("Employee department cannot be null", HttpStatus.BAD_REQUEST, employee);
        } else if (employee.getAge() ==0) {
            return builder.responseBuilder("Employee Age cannot be 0", HttpStatus.BAD_REQUEST, employee);
        } else {
            employee.getAge();
        }
    ...
    public ResponseEntity<Object> updateEmployee(Long id, Employee employee) {
        ...
        return builder.responseBuilder("Employee Update Successfully", HttpStatus.OK, employeeOptional.get());
    }
